...
```csharp
            var activeSessionResult = await beamClient.CreateSessionAsync("your-entity-id");
...
``` csharp
            var activeSessionResult = await beamClient.GetActiveSessionAsync("your-entity-id");
            if (activeSessionResult.Status == BeamResultType.Success)
            {
                var session = activeSessionResult.Result;
