...
One of the less obvious use-cases for profiles is using them for escrow purposes, instead of using them for managing player assets..

A common scenario in web3 gaming are X games. Every player can participiate in a PvP-match, but has to put one of their assets on the line - and you don't want players to be able to simply disconnect or moving their assets through the companion app right before they loose a match.
...
Let's go through the steps you would need to take in order to properly set this up within by providing an actual scenario. In our scenario, Player A wants to create a 1v1 PvP match where the winner takes all. Player A goes through your game it's UI that allows them to create a match.

The first step you would need to do is to create a match. Please note that since the match is a new profile, it's important to track which player (in our case: player A) initiated the match, and to store the match-profile-ID somehwere with the player-profile-ID.
