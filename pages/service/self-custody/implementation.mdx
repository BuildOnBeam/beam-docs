...
The implemenation of our new self-custodial service consists of two components
...
In the near future, we expect to expand the client side component with all the server client methods, so that you also have the option to implement completley in the client.
...
Keep in mind that we expect you to use the `Publishable` API key currently, on both the client-side as well as the server client when interacting with the self-custody service.
...
<Cards>
  ...
  <Card
    icon="✅ "
    title="C# (RestSharp)"
    href="https://www.nuget.org/packages/BeamSelfCustodial"
  />
</Cards>