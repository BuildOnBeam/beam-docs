...
- **Minting fee:** The fee for minting Node Tokens is payable in BEAM, creating constant demand for the network's native token. Node Tokens that are minted with other currencies will be autonomously swapped into BEAM and later burned.
...
The Node Token exists through an ownerless and autonomous smart contract that has been thoroughly audited for security. The smart contract handles all token operations autonomously, including:
