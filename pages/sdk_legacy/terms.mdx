...
These terms and conditions (“**Terms**”) constitute a legally binding agreement between Beam Labs (“**Licensor**”) and you (“**Licensee**”) in relation to the Licensee's use of the Beam application programming interfaces, including the 'Player API' and 'Automation API' (“**Beam APIs**”). 

**IMPORTANT: Please read these Terms carefully before registering to use or using the Beam APIs, as they govern the Licensee's use of the Beam APIs that are part of the Beam software development kit (“Beam SDK”). This includes Licensee's rights and obligations and Licensor's disclaimers and limitations of legal liability relating to the Licensee's use of, and access to, the Beam APIs.** 
...
The Beam APIs are a group of application programming interfaces (made available through technology developed by [Openfort](https://www.openfort.xyz/)) which allow licensees to interface with endpoints allowing them to execute instructions directly on a variety of EVM compatible blockchain networks (including to create and interact with blockchain smart contract accounts and to enable users of Licensee Games (as defined below) to create and use self-custodial smart contract accounts) that may be integrated into Licensee Games (as defined below) by use of one or more unique electronic keys (the “**API Keys**”). 

**2.2 License to use Beam APIs and API Keys**

Subject to the Licensee's compliance with these Terms, Licensee may create the API Keys and Licensor grants the Licensee a limited, non-exclusive, non-sublicensable, non-transferable, and revocable license for the Licensee to (i) use one or more API Keys to access the Beam APIs, (ii) use the Beam APIs to establish an integration between the Beam APIs and gaming related applications of Licensee (“**Licensee Game**”), and (iii) authorize players of Licensee Games to access the Beam APIs from the Licensee Games, in each instance solely as permitted hereunder (collectively, the “**License**”). Although the License is currently free for the Licensee to use, Licensor reserves the right to charge a fee for use of it in the future. 

Licensor may modify and update the Beam APIs from time to time. Licensee must alter all Licensee Games to implement and use the most current version of the Beam APIs at Licensee's own cost as soon as commercially practicable after any modification or update, and no later than within 60 days following notice of a modification or update. In case the modification or update is necessary to patch a security flaw which could have a material adverse effect on the Licensee Games or its players or the Licensee, the Licensee must promptly implement such a modification and update.

**2.3 Smart contract accounts available by use of the Beam APIs**

There are two types of smart contract accounts available through the Beam APIs. 

**Player API**: The first is made available through the 'Player API' solution, which enables Licensee to let players of Licensee Games create a self-custodial smart contract account. The creation of a public/private key pair and account abstraction, in addition to any subsequent blockchain transaction by a player, will occur by use of a social login or other authentication method (if applicable) by the player (client-side authentication). The private key created through the social login or other authentication method (as applicable) will be split into three different pieces (shards) and each will be stored in a different location. Only the user will be able to put together the pieces to sign a transaction.  

**Automation API**: The second is made available through the 'Automation API' solution, which enables Licensee to create smart contract accounts that are controlled by the Licensee (may e.g. be used for programmatic distributions of Tokens pertaining to the Licensee Game). The 'Automation API' may not be used to hold custody of the assets (including Tokens) of third parties. 

Neither Licensor nor Licensee will hold custody over any third-party assets by using the Beam APIs and will not by virtue of using the Beam APIs have knowledge of any third-party private key created by use of the Beam APIs. By using the Beam APIs, neither Licensor nor Licensee will be able to conduct, execute, or effectuate any transactions on anyone's behalf. Licensor is not a wallet provider, exchange or broker. 

**3. Handling of API Keys**

The Licensee will be required to use the API Keys to access the Beam APIs. The Licensee will treat the API Keys confidentially and not share the API Keys with any third party without Licensor's prior written consent. The Licensee may not sell, sublicense, pledge, lease or otherwise transfer or disclose the API Keys received to any other party. Licensee agrees to notify Licensor immediately if the Licensee believes that API Key has been compromised. Licensee agrees that it will be responsible for the use of the API Keys. 
...
In addition to what may be stated elsewhere in these Terms, the following obligations and restrictions apply to the Licensee:
  ...
  a) The Licensee will provide Licensor with accurate information (as may be required) about the Licensee if reasonably requested by Licensor.   
     
  b) The Licensee will only provide access to the Beam APIs to (i) employees or current contractors of Licensee (such persons must comply with these Terms and the Licensee will be responsible and liable for any non-compliance on their behalf), and (ii) players of the Licensee Games.   
  ...
  h) The Licensee will not conduct any activity that implies infringement or violation of any intellectual property rights owned by Licensor or another third-party.   
     
  i) The Licensee will (i) use commercially reasonable efforts to prevent unauthorized access to or use of the Beam APIs and API Keys and notify Licensor promptly of any such unauthorized access or use, and (ii) use the Beam APIs and API Keys only in accordance with the documentation provided by Licensor and all applicable laws, rules, and regulations.  
...
The Licensee represents and warrant to Licensor that (a) if the Licensee is entering into these Terms as an individual, that the Licensee is of legal age to form a binding contract and that the Licensee otherwise has the legal capacity to enter into these Terms and be bound by them, or (b) if the Licensee is entering into these Terms as an entity, that the Licensee is duly organized, validly existing and in good standing under the laws of the jurisdiction in which it is organized and that the person accepting these Terms has all requisite power to accept these Terms on that entity's behalf.

**6. Termination**

Licensor reserves the right to terminate or limit any user's status or access to or use of the API Keys or Beam APIs at any time without any liability if Licensee is in breach of these Terms or if Openfort's technology is no longer available to Licensor. 

The Licensee may terminate these Terms by ceasing to use the Beam APIs, and Beam API Keys. In the event of such a termination, the Licensee shall remove implementation of the Beam APIs and any copies of the API Keys in Licensee's possession or control.
...
The Licensee may not assign or transfer any right to use the Beam APIs or API Keys or any of Licensee's rights or obligations under these Terms without Licensor's express prior written consent, including by operation of law or in connection with any change of control. Licensor may assign or transfer any or all of its rights or obligations under these Terms, in whole or in part, with or without notice or obtaining the Licensee's consent or approval.  
