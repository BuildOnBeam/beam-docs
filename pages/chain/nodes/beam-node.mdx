---
description: 'Guide to setting up a Beam Node on the Beam testnet, detailing installation and monitoring steps for optimum node functionality.'
---
...
To get the latest network upgrades we will get the network upgrade file `upgrade.json` and place it into the correct directory. If you haven't used the installer your config directory might be in a different location, the file should be placed in `{chain-config-dir}/2tmrrBo1Lgt1mzzvPSFt73kkQKFas5d1AP88tv9cicwoFp8BSn/upgrade.json` where `2tmrrBo1Lgt1mzzvPSFt73kkQKFas5d1AP88tv9cicwoFp8BSn` is the `{blockchainID}`.
